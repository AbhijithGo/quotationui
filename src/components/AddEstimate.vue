<template>
  <div @click="anywhereClick">
    ...
    <b-container fluid class="card estimateContainer margBot" id="ename_enum_container">
      <b-row class="my-1">
        <b-col sm="4">
          <label class="label" id='estimateName'>
            Estimate Name:
        ...
        <b-col sm="4">
          <label class="label" id='estimateNumber'>
            Estimate number:
            <span class="text-danger">*</span>
          </label>
          <label class="inputContainer">
      ...
      <b-row class="my-1">
        ...
        <b-col sm="3" id="footer">
          ...
          <!-- <b-form-input
            ...
            required
          ></b-form-input> -->
          <b-form-textarea id="memotextarea_1" required v-model="eFooter" placeholder="Footer.." max-rows="3"></b-form-textarea>
        </b-col>
        <b-col sm="3" id="memo">
          <label class="label" >
            Memo:
          ...
          <b-form-textarea id="memotextarea_2" required v-model="eMemotext" placeholder="Memo..." max-rows="3"></b-form-textarea>
        </b-col>
      </b-row>
    </b-container>
    <b-container fluid class="card estimateContainer" style="overflow-x:auto;" id="eTable">
      <div id="resp-table">
        <div id="resp-table-header">
          ...
          <div id="itemType" class="table-header-cell">Item Type</div>
          ...
          <div class="resp-table-row" v-for="(row, i) in rows" track-by="i" :key="i">
            ...
            <div class="table-body-cell itemType">
              ...
              >
               ...
               <div class="dateError" v-if="row.itemTypedPickedBool">Please, Select item type!!!</div>
            </div>
            <div class="table-body-cell item">
              <b-form-input
                ...
                aria-required="false"
              ></b-form-input>
                <!-- @keyup.enter="toggleSearch(i)" -->
               <div class="dateError" v-if="row.searchBool">Please, Enter some value!!!</div>
            ...
            <div class="table-body-cell search">
              <b-button class='searchButton' @click="toggleSearch(i)"><b-icon class='serachIconCustom' icon="search"></b-icon></b-button>
            </div>
      ...
      <hr/>
        ...
        <modal name="dataTable" top=21 height=700 width=700>
      ...
      <b-row class="my-1 tableBottomContainer" id="save">
        <b-button  type="submit" @click="save" class="saveFormBtn" ref="formSubmitBtnEStimates">
          <b-icon-plus class="customPlusIcon_save"></b-icon-plus>
          <span class="addBtnTxt_save">Save</span>
        </b-button>
        <b-button id="test"  @click="reset" class="saveFormBtn resetFormBtn">
          <!-- <b-icon-plus class="customPlusIcon_save"></b-icon-plus> -->
    ...
    <v-tour name="myTour2" :steps="steps" :options="{ highlight: true }"></v-tour>
  </div>
</template>
...
export default {
  name: 'my-tour2',
  props: ['editRowData', 'toggleRequest', 'tableData', 'visible','tour'],
  beforeDestroy () {
  ...
  mounted () {
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Enter' && this.visible) {
        const elem = this.$refs.formSubmitBtnEStimates
        elem.click()
      }
    })
  ...
  created () {
    ...
    if (!this.toggleRequest) {
      ...
      this.eNumber = this.tableData[0].next_estimate_no
  ...
  methods: {
    ...
    select (element) {
      ...
      this.$modal.hide('dataTable')
    },
    toggleSearch (i) {
      if (this.rows[i].itemSelected === '') {
        this.rows[i].searchBool = true
        return
      }
      this.$modal.show('dataTable')
      this.rowIndex = i
      ...
      axios
        ...
        .then(response => {
          if (response.data.length === 0) {
            this.$alert('No results found!!!!', 'Error', 'error').then(() => {
              this.rows[i].description = ''
              this.rows[i].quantity = 0
              this.rows[i].price = 0
              this.rows[i].unit = ''
              this.rows[i].discount = 0
              this.rows[i].total = 0
              this.overSubAllTotal = 0
              this.overAllTotal = 0
              this.overAllDiscount = 0
              this.rows[i].itemSelectedId = ''
              this.calculateTotal(i)
              this.$modal.hide('dataTable')
            })
          } else {
    ...
    validateEnum () {
      const check = (element) => element.quotation_id.toLowerCase() === this.eNumber.toLowerCase()
    ...
    initialValues () {
      return {
        steps: [
          {
            target: '#eName',  // We're using document.querySelector() under the hood
            content: `Enter An estimate Name`,
             params: {
              placement: 'right'
            }
          },
          {
            target: '#eNumber',
            content: 'Enter an Estimate Number',
            params: {
              placement: 'right'
            }
          },
          {
            target: '#eCustomer',
            content: 'Select A customer from the dropdown once you start typing',
            params: {
              placement: 'right'
            }
          },
          {
            target: '#edateCol',
            content: 'Select an Invoice Date',
            params: {
              placement: 'right'
            }
          },
          {
            target: '#eSubheading',
            content: 'Select a Subheading for the estimate',
            params: {
              placement: 'right'
            }
          },
          ...
          {
            target: '#eSalesMan',
            content: 'Select a Salesman from the List',
            params: {
              placement: 'right'
            }
          },
          {
            target: '#eDate_expiryCol',
            content: 'Select an Expiry Date',
            params: {
              placement: 'right'
            }
          },
      ...
      if (this.overAllTotal !== 0 && this.presentIdBool === false && this.customerSelected !== '' && this.eDate !== '' && this.eDate_expiry !== '' && this.eSalesManSelected !== '' && !itemTypedPickedBool && !itemSelectedBool) {
        ...
        if (this.toggleRequest) {
          ...
          axios
            ...
            .then(res => {
              // console.log(res)
              // console.log('in else toggle req')
              this.isSucess = true
            })
            .catch(error => {
              console.log(error)
        ...
        if (this.presentIdBool) {
          alert('Estimate number should not be same !!!!')
        }
      }
    },
    deleteRow (index) {
    ...
    itemChangeInput (index) {
      if (this.rows[index].itemTypedPicked === '') {
        this.rows[index].itemTypedPickedBool = true
      }
      if (this.rows[index].itemSelected.length > 0) {
        this.rows[index].searchBool = false
      }
    ...
    getselectedValue (index) {
      let selectedType = this.rows[index].itemTypedPicked
      this.rows[index].itemTypedPickedBool = false
      // axios
...
#memotextarea_1:focus, #memotextarea_2:focus {
